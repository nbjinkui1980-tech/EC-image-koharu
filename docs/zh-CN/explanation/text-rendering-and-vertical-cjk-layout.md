...
- `LineBreaker` 仅在允许自动换行的路径（包括 `AllText`）中使用 ICU4X；识别到目标语言的 `HanOnly` 路径保留译文中的显式行，不自动换行或断词
...
### HanOnly 与 AllText 的排版差异

- `HanOnly` 且目标语言有效时，自动字号把源字号作为搜索上限：英语优先使用检测字号并减 5 px，日文、韩文保持 0 偏移，其他已识别语言减 5 px；手动字号始终优先且不参与自动分组
- 这一路径保留显式行，不软换行或拉伸字形；自动节点会选择完整字形可放入 Source 安全区域的最大字号，同一水平组统一使用组内最小安全字号。合成前继续检查页面、Source、受保护原文、其他节点和目标精灵重叠；最小字号仍无法安全放置时原子失败
- `AllText` 继续使用原有的气泡区域、自动适配、回流和变换行为；目标语言缺失或无效时，`HanOnly` 也保留原有自动适配行为
...
- 支持纵排 CJK 常见的自右向左列流
- 在允许自动换行的路径中用 ICU4X 做断行，而不是手搓一个按字符分割的换行器
...
### 自动换行路径仍使用 ICU 默认 CJK 断行

`AllText` 等允许自动换行的路径仍使用 ICU4X 默认断行，目前未针对漫画 kinsoku 规则定制；固定显式行的 `HanOnly` 路径不执行自动断行。
