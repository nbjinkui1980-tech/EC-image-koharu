...
OpenAI 兼容端点可以同时提供两个彼此独立的模型目标：

- 翻译模型只接收 OCR 文本并生成目标语言译文。
- 智能排版模型接收缩放后的页面参考图、目标文本块的几何与当前样式、译文和有限字体列表，生成换行与现有样式字段的规划。

两者共用 **Settings > API Keys > OpenAI Compatible** 中唯一的 Base URL 和可选 API key，但分别选择模型。切换排版模型不会卸载或替换当前翻译模型。排版任务每次运行都会读取最新连接配置；如果修改了共享 Base URL 或 API key，排版会在下一次运行时使用新值，已经加载的翻译模型则要经过现有的重新加载流程才会使用新连接。

自动排版开关只控制 Full Pipeline 和单个文本块的 `Generate` 是否自动加入排版阶段。画布工具栏的 `智能排版` 是独立手动入口，即使自动排版关闭，只要共享 Base URL、排版模型和 Planner 引擎配置完整，仍可运行。

智能排版不会让云端模型直接修改最终图片。模型返回的整页方案必须先通过严格校验，之后仍由本地 Rust Renderer 使用真实字体生成像素。任意节点缺失、重复或越界，译文被篡改，字体不在系统/已缓存白名单中，或样式数值不安全时，整页方案都会原子拒绝：Scene 不发生部分修改，任务显示 warning，Renderer 继续用原译文和样式出图。

当前限制：

- 只支持 `OpenAI Compatible` 目录中的排版模型；目录本身不标注视觉能力，选中的模型必须实际支持图像输入。
- 只规划显式换行、字体、字号上限、颜色、粗斜体、描边和对齐；不支持字距、行高、任意坐标移动、阴影或渐变。
- 只会选择系统字体或已经缓存的字体，不会因为规划结果自动下载字体。
- 使用远程智能排版时，缩放后的页面参考图及目标文本块信息会发送到配置的端点；这与只发送 OCR 文本的远程翻译不同。
...
!!! note
    ...
    启用远程智能排版时，还会发送缩放后的页面参考图、目标译文、文本框几何和当前样式。最终像素仍由本地 Renderer 生成。
