...
在 macOS 上，调试构建会把提供方和 Codex 凭据保存在 `~/.koharu-dev/secrets/`，并设置为仅所有者可访问。这个仅用于开发的存储与非调试构建使用的原生 Keychain 完全隔离，因此临时签名的重复构建不会触发 Keychain 授权弹窗。首次使用此边界后的调试构建会显示为未登录，需要重新输入开发凭据。已有的 `koharu` Keychain 项不会自动迁移或删除；仅在确定没有非调试构建继续使用时才手动清理。
