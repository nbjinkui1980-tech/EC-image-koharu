...
macOS のデバッグビルドでは、provider と Codex の認証情報を所有ユーザーだけが読める `~/.koharu-dev/secrets/` に保存します。この開発専用ストアは、非デバッグビルドが使用するネイティブ Keychain から分離されているため、アドホック署名で再ビルドしても Keychain の認証ダイアログは表示されません。この境界を導入した後の最初のデバッグ起動では未ログイン状態になり、開発用認証情報の再入力が必要です。既存の `koharu` Keychain 項目は自動移行も自動削除もされません。非デバッグビルドで不要になったことを確認してから手動で削除してください。
