...
mod source_gate_selection {
            ...
            .filter(|entry| entry.schema.role == phase_role(environment.phase))
        {
            ...
            for (device, vl, vl_evidence, process) in &mut runners {
                for (candidate_id, policy) in &selected {
                    ...
                    if environment.r51_formal_custody.is_some() {
                        let scene_eligible_line_quads = r51_scene_eligible_line_quads(
                            scene
                                .page(page)
                                .ok_or_else(|| invalid_data("R51 scene page is missing"))?,
                        )?;
    ...
    fn r51_selected_support_from_diagnostics(
        ...
        for event in diagnostics {
            ...
            for target in targets {
                ...
                require(
                    !target.eligible_line_quads_f32_bits.is_empty(),
                    "R51 emitted target has no eligible line geometry",
                )?;
                for line in &target.eligible_line_quads_f32_bits {
                    let line_rect = r51_quad_bits_rect(*line)?;
                    for (pixel, addition) in support
                        .iter_mut()
                        .zip(r51_rect_mask(width, height, line_rect))
                    {
                        *pixel |= addition;
                    }
                }
    ...
    fn r51_scene_eligible_line_quads(page: &Page) -> io::Result<HashSet<[u32; 8]>> {
        ...
        for node in page.nodes.values() {
            ...
            let Some(lines) = eligible_text_lines(&node.transform, text, page.width, page.height)
            else {
                continue;
            };
            for quad in lines
                .iter()
                .flat_map(|line| line.region.line_polygons.iter().flatten())
            {
                quads.insert([
                    quad[0][0].to_bits(),
                    quad[0][1].to_bits(),
    ...
    fn r51_detector_diagnostics(
        ...
        oracle: &OracleValidatedEntry,
        diagnostics: &[SourceGateDiagnosticEvent],
        scene_eligible_line_quads: &HashSet<[u32; 8]>,
        rejection_reason: &Option<String>,
    ) -> io::Result<(
        ...
        let mut support_safety_passed = true;
            ...
            for detector in raw_detectors {
                ...
                let (
                    ...
                    Some(SourceGateDetectorAssignmentDiagnostic::Target { target_index }) => {
                        ...
                        let eligible = ownership
                            .and_then(|value| value.eligible_text_line_quad_f32_bits)
                            .ok_or_else(|| {
                                invalid_data("R51 selected detector has no eligible line geometry")
                            })?;
                        require(
                            geometry.eligible_line_quads_f32_bits.contains(&eligible),
                            "R51 selected detector eligible geometry drift",
                        )?;
                        (
                            Some(r51_target_id_for_rect(schema, oracle, emitted_rect)?),
                            "selected_han",
                            scene_eligible_line_quads
                                .contains(&eligible)
                                .then_some(eligible),
                    ...
                    Some(SourceGateDetectorAssignmentDiagnostic::Protected { protected_index }) => {
                        ...
                        let eligible = ownership
                            .and_then(|value| value.eligible_text_line_quad_f32_bits)
                            .ok_or_else(|| {
                                invalid_data("R51 protected detector has no eligible line geometry")
                            })?;
                        require(
                            geometry.eligible_line_quads_f32_bits.contains(&eligible),
                            "R51 protected detector eligible geometry drift",
                        )?;
                        (
                            None,
                            "preserved_source",
                ...
                let protected_support_pixels = detector_mask
                    ...
                    .count();
                let unsupported_rotation_selected =
                    !result.derived.selected_rotation_target_ids.is_empty();
                let unmatched_selected_nodes = &result.derived.unmatched_selected_node_ids;
                support_safety_passed &= match canonical_assignment {
                    "selected_han" => {
                        line_support_equals_detector
                            && agreed_mask_subset
                            && protected_support_pixels == 0
                            && !unsupported_rotation_selected
                            && unmatched_selected_nodes.is_empty()
                ...
                let preimage = serde_json::json!({
                    ...
                    "unsupported_rotation_selected": unsupported_rotation_selected,
                    "unmatched_selected_nodes": unmatched_selected_nodes,
    ...
    fn write_r51_cell_diagnostic(
        ...
        result.derived.passed &= support_safety_passed;
        let passed = result.derived.passed;
        let terminal_reason = (!passed).then(|| {
            rejection_reason.clone().unwrap_or_else(|| {
                if support_safety_passed {
                    "coverage_failure".into()
                } else {
                    "detector_support_safety_failure".into()
                }
            })
        });
...
"#;
    ...
    fn r51_selection_geometry_closes_detector_ownership_preimages() {
        ...
        let scene_eligible_line_quads = HashSet::from([target_bits]);
        let (_, _, _, records, safety_passed) = r51_detector_diagnostics(
            &environment,
            &result,
            &schema,
            &oracle,
        ...
        let (_, _, _, _, missing_scene_safety_passed) = r51_detector_diagnostics(
            &environment,
            &result,
            &schema,
            &oracle,
        ...
        .unwrap();
        assert!(!missing_scene_safety_passed);
        ...
        unsafe_result
            ...
            .push("target".into());
        let (_, _, _, _, unsafe_safety_passed) = r51_detector_diagnostics(
            &environment,
            &unsafe_result,
            &schema,
            &oracle,
        ...
        .unwrap();
        assert!(!unsafe_safety_passed);
        let rejected_reason = Some("pp_vl_incomplete_coverage".to_owned());
        let (_, _, _, rejected, rejected_safety_passed) = r51_detector_diagnostics(
            &environment,
            &result,
            &schema,
            &oracle,
    ...
    fn r51_selected_and_downstream_support_have_independent_geometry_sources() {
        ...
        let diagnostics = vec![SourceGateDiagnosticEvent::SelectionGeometry {
            ...
            targets: vec![SourceGateTargetGeometryDiagnostic {
                ...
                eligible_line_quads_f32_bits: vec![r51_test_quad_bits(10.0, 10.0, 20.0, 20.0)],
            }],
